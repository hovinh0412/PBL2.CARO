...
void _Board::drawBoard()
{
	...
	// ##########################
	// Cot thuong
	for (int i = 1; i < _size * 2; i = i + 2)
	{
		for (int j = 0; j < _size * 4; j = j + 4)
		{
			// Sleep(1);
			_Common::gotoXY(j, i);
			if (j == 0)
	...
	_Common::gotoXY(0, 0);
	cout << char(201);
	_Common::gotoXY(_size * 4, 0);
	cout << char(187);

	// Ve dong cuoi
	_Common::gotoXY(0, _size * 2);
	for (int i = _size * 4; i > 0; i--)
	{
		Sleep(1);
		if (i % 4 == 0)
			cout << char(207);
	...
	_Common::gotoXY(0, _size * 2);
	cout << char(200);
	_Common::gotoXY(_size * 4, _size * 2);
	cout << char(188);
...
long _Board::SoDiemTanCong_DuyetNgang(long Dong, long Cot, const long Defend_Score[], const long Attack_Score[])
{
	...
	iScoreTempNgang -= Defend_Score[iSoQuanTa + 1];
...
long _Board::SoDiemTanCong_DuyetDoc(long Dong, long Cot, const long Defend_Score[], const long Attack_Score[])
{
	...
	iScoreTempDoc -= Defend_Score[iSoQuanTa + 1];
...
long _Board::SoDiemTanCong_DuyetCheo1(long Dong, long Cot, const long Defend_Score[], const long Attack_Score[])
{
	...
	iScoreTempCheoNguoc -= Defend_Score[iSoQuanTa + 1];
...
long _Board::SoDiemTanCong_DuyetCheo2(long Dong, long Cot, const long Defend_Score[], const long Attack_Score[])
{
	...
	iScoreTempCheoXuoi -= Defend_Score[iSoQuanTa + 1];
...
// AI(hard)
long _Board::SoDiemTanCong2_DuyetNgang(long Dong, long Cot, const long Defend_Score[], const long Attack_Score[])
{
	long iScoreTempNgang = 0;
	long iScoreAttack = 0;
	int iSoQuanTa = 0;
...
long _Board::SoDiemTanCong2_DuyetDoc(long Dong, long Cot, const long Defend_Score[], const long Attack_Score[])
{
	long iScoreTempDoc = 0;
	long iScoreAttack = 0;
	int iSoQuanTa = 0;
...
long _Board::SoDiemTanCong2_DuyetCheo1(long Dong, long Cot, const long Defend_Score[], const long Attack_Score[])
{
	long iScoreTempCheoNguoc = 0;
	long iScoreAttack = 0;
	int iSoQuanTa = 0;
...
long _Board::SoDiemTanCong2_DuyetCheo2(long Dong, long Cot, const long Defend_Score[], const long Attack_Score[])
{
	long iScoreTempCheoXuoi = 0;
	long iScoreAttack = 0;
	int iSoQuanTa = 0;
...
long _Board::SoDiemPhongThu2_DuyetDoc(long Dong, long Cot, const long Defend_Score[], const long Attack_Score[])
{
	long iScoreTempDoc = 0;
	long iScoreDefend = 0;
	int iSoQuanDich = 0;
...
long _Board::SoDiemPhongThu2_DuyetNgang(long Dong, long Cot, const long Defend_Score[], const long Attack_Score[])
{
	long iScoreTempNgang = 0;
	long iScoreDefend = 0;
	int iSoQuanDich = 0;
...
long _Board::SoDiemPhongThu2_DuyetCheo1(long Dong, long Cot, const long Defend_Score[], const long Attack_Score[])
{
...
long _Board::SoDiemPhongThu2_DuyetCheo2(long Dong, long Cot, const long Defend_Score[], const long Attack_Score[])
{
	long iScoreTempCheoXuoi = 0;
	long iScoreDefend = 0;
	int iSoQuanDich = 0;
...
_Point _Board::Tim_Kiem_NuocDi()
{
	...
	for (int i = 0; i < _size; i++)
	{
		for (int j = 0; j < _size; j++)
		{
			...
			{
				DiemTanCong += SoDiemTanCong_DuyetDoc(i, j, Defend_Score, Attack_Score);
				DiemTanCong += SoDiemTanCong_DuyetNgang(i, j, Defend_Score, Attack_Score);
				DiemTanCong += SoDiemTanCong_DuyetCheo1(i, j, Defend_Score, Attack_Score);
				DiemTanCong += SoDiemTanCong_DuyetCheo2(i, j, Defend_Score, Attack_Score);

				DiemPhongThu += SoDiemPhongThu_DuyetDoc(i, j, Defend_Score, Attack_Score);
				DiemPhongThu += SoDiemPhongThu_DuyetNgang(i, j, Defend_Score, Attack_Score);
				DiemPhongThu += SoDiemPhongThu_DuyetCheo1(i, j, Defend_Score, Attack_Score);
				DiemPhongThu += SoDiemPhongThu_DuyetCheo2(i, j, Defend_Score, Attack_Score);
...
_Point _Board::Tim_Kiem_NuocDi2()
{
	_Point Oco;
	int dong = 0, cot = 0;
	long Diem = 0;
	for (int i = 0; i < _size; i++)
	{
		for (int j = 0; j < _size; j++)
		{
			...
			if (_pArr[i][j].getCheck() == 0)
			{
				DiemTanCong += SoDiemTanCong2_DuyetDoc(i, j, Defend_Score2, Attack_Score2);
				DiemTanCong += SoDiemTanCong2_DuyetNgang(i, j, Defend_Score2, Attack_Score2);
				DiemTanCong += SoDiemTanCong2_DuyetCheo1(i, j, Defend_Score2, Attack_Score2);
				DiemTanCong += SoDiemTanCong2_DuyetCheo2(i, j, Defend_Score2, Attack_Score2);

				DiemPhongThu += SoDiemPhongThu2_DuyetDoc(i, j, Defend_Score2, Attack_Score2);
				DiemPhongThu += SoDiemPhongThu2_DuyetNgang(i, j, Defend_Score2, Attack_Score2);
				DiemPhongThu += SoDiemPhongThu2_DuyetCheo1(i, j, Defend_Score2, Attack_Score2);
				DiemPhongThu += SoDiemPhongThu2_DuyetCheo2(i, j, Defend_Score2, Attack_Score2);
